...
const addListeners = () => {
    ...
    if(els) {
        els.forEach(e => e.addEventListener("click", e => {
            gameOverClicked(e.srcElement.getAttribute("type"));
        }));
    }
}
...
const makeDiv = className => {
    ...
    div.classList.add("icon-frame");
    return div;
}


const gameOverClicked = method => {
...
const clearGameOver = () => {
    ...
    kings.forEach(e => {
        ["winner","checkmate","surrender","surrender-2","surrender-3","draw", "timeout"].forEach(className => {
            e.classList.remove(className);
            if (e.querySelector(".status-bubble")) {
                e.querySelector(".status-bubble").remove();
            }
...
const gameOver = method => {
    ...
    kings.forEach(e => {
        ...
        if(method == "timeout") {
            if(e.classList.contains("wk")) {
                statusBubble.classList.add("winner");
                e.classList.add("winner");
            } else {
                statusBubble.classList.add("timeout");
                ["clock", "finger"].forEach(name => {
                    let el = makeDiv(name);
                    icon.appendChild(el);
                });
        ...
        if(method == "surrender-2") {
            if(e.classList.contains("wk")) {
                statusBubble.classList.add("surrender-2");
                e.classList.add("surrender-2");
                ["stick", "flag"].forEach(name => {
                let el = makeDiv(name);
                icon.appendChild(el);
            });
        ...
        if(method == "surrender-3") {
            if(e.classList.contains("wk")) {
                statusBubble.classList.add("surrender-3");
                e.classList.add("surrender-3");
                ["stick", "flag"].forEach(name => {
                let el = makeDiv(name);
                icon.appendChild(el);
            });
